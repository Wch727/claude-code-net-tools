...
工具按 query/provider 轮询合并并去重，默认不启发式重排。对于中文全名或精确实体，工具先搜索抽出的实体原文；只要结果集中存在包含完整实体的结果，就确定性地过滤不含该实体的噪音。`intent=academic` 遇到精确论文标题时会把精确命中放在模糊结果前。
...
- 免费搜索引擎可能限速、返回验证码或调整 HTML；浏览器模式也不保证一定通过反机器人检查。验证页会被标记为失败，不会当作正常结果或正文。
