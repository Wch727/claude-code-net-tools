...
```text
当用户问题需要外部信息时，优先使用 Claude Code 内置 WebSearch 和 WebFetch；它们不可用、报错、拒绝域名、内容不完整，或者任务需要可配置搜索源、长文档分页、PDF 提取、会话、代理或真实浏览器时，再使用 net-tools。内置工具已经明确失败后，直接切换到 net-tools，不重复等价的内置调用。

你负责理解问题和准备查询；需要 net-tools 时，由它负责搜索与读取：
